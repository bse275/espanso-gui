...
None of the builds are code-signed, so the OS will warn on first launch.

- **Windows:** SmartScreen → **More info → Run anyway**.
- **macOS:** after dragging the app to Applications, either Right-click → **Open**, or clear the download quarantine in Terminal (do not use `codesign --deep` — it breaks Electron's nested helpers):

```bash
xattr -cr "/Applications/Espanso GUI.app"
```

If macOS still refuses to open it:

```bash
codesign --force --sign - "/Applications/Espanso GUI.app"
xattr -cr "/Applications/Espanso GUI.app"
```
